...
def main():
  ...
  # 使用脚本目录下的测试集路径
  test_dir(os.path.join(SCRIPT_DIR, 'base'))

  extend_dir = os.path.join(SCRIPT_DIR, 'extend')
  for dir in os.listdir(extend_dir):
    if (group == 0 and not dir.startswith('not')) or dir == str(group) or \
       (group != 0 and dir.startswith('not') and dir != f'not{group}'):
      test_dir(os.path.join(extend_dir, dir))
...
def test_advance():
  for path, _, files in os.walk(os.path.join(SCRIPT_DIR, 'advance')):
    for file in sorted(filter(lambda x: getsub(x) == 'cmm', files)):
      file = os.path.join(path, file)
      result['total'] += 1
